...
This is a Python based pipeline for the automated reduction of Swift BAT survey data using HEASoft to get the light curves. The pipeline performs the sequence of BAT processing tasks required to generate background subtracted light curves in user defined energy bins assuming they are allowed by the instrument in its 14-195 keV range. This pipeline assumes the user has already downloaded Swift BAT survey observations (check **Requirements** section).
...
- Writing the header of this merged file.
...
2. Run the pipeline for same source, for the observational data within time duration 2018-03-03 to 2018-07-21 (only accepted format is yyyy-mm-dd), with energy bins 14-51.1, 51.1-194.9 keV (Note: the user need not provide exact values as the code will automatically snap the values to allowed energy bins as seen in command below), with time bins of 2 days, and overwrite the previous results.
